...
const unpackInputPluginOptions = ({
  ...
  return unpackOptions({
    ...rest,
    skipPreflightCheck,
    babelHelpers: rest.babelHelpers || transformCode.BUNDLED,
    caller: {
...
const WORKER_PATH = url.fileURLToPath(new URL('./worker.js', (typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('index.js', document.baseURI).href))));
function createParallelWorkerPool(parallel, overrides) {
  if (typeof parallel === 'number' && (!Number.isInteger(parallel) || parallel < 1)) {
    throw new Error('The "parallel" option must be true or a positive integer specifying the number of workers.');
  }
  if (!parallel) return null;
  if (overrides !== null && overrides !== void 0 && overrides.config) {
  ...
  const workerCount = typeof parallel === 'number' ? parallel : Math.min(os.cpus().length, 4);
...
function executeWithWorkerPool(workerPool, context, taskOpts, babelOptions) {
  ...
  if (nonSerializableKey) {
    return Promise.reject(new Error(`Cannot use "parallel" mode because the "${nonSerializableKey}" option is not serializable.`));
  }
...
function createBabelInputPluginFactory(customCallback = returnObject) {
  ...
  return pluginOptions => {
    ...
    const extensionRegExp = new RegExp(`(${extensions.map(transformCode.escapeRegExpCharacters).join('|')})(\\?.*)?(#.*)?$`);
    ...
    const helpersFilter = {
      id: new RegExp(`^${transformCode.escapeRegExpCharacters(transformCode.HELPERS)}$`)
    };
    return {
      name: 'babel',
      resolveId: {
      ...
      transform: {
        ...
        async handler(code, filename) {
          ...
          if (filename === transformCode.HELPERS) return null;
          const resolvedBabelOptions = {
            ...babelOptions,
            filename
          };
          if (workerPool) {
            return executeWithWorkerPool(workerPool, this, {
              inputCode: code,
    ...
    return {
      ...
      renderStart(outputOptions) {
        ...
        if (extensions || include || exclude) {
          transformCode.warnOnce(this, 'The "include", "exclude" and "extensions" options are ignored when transforming the output.');
        }
